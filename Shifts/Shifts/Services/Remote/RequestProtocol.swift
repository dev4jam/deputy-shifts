...
public protocol RequestProtocol {
    ...
    /// - Returns: URL
    func url(in service: NetworkServiceProtocol) throws -> URL
...
public extension RequestProtocol {
    func headers(in service: NetworkServiceProtocol) -> HeadersDict {
        ...
        if let auth = service.auth {
            switch auth {
            case .token(let token):
                params["Authorization"] = "Deputy " + token
    ...
    func url(in service: NetworkServiceProtocol) throws -> URL {
        ...
        let baseURL = service.url.appending(self.endpoint)
    ...
    public func urlRequest(in service: NetworkServiceProtocol) throws -> URLRequest {
        ...
        let timeout = self.timeout ?? Config.defaultTimeout
...
extension Dictionary where Key == String, Value == Any {
    ...
    public func urlEncodedString(base: String = "") throws -> String {
        ...
        let items: [URLQueryItem] = self.compactMap { (key,value) in
            let safeValue = String(describing: value).escapedString()
            
            return URLQueryItem(name: key, value: safeValue)
        }
    ...
    func fill(withValues dict: [String: Any?]?) -> String {
        ...
        data.forEach { arg in
            if let unwrappedValue = arg.value {
                if let safeValue = String(describing: unwrappedValue).escapedString() {
                    finalString = finalString.replacingOccurrences(of: "{\(arg.key)}", with: safeValue)
                }
            }
        }
        return finalString
...
extension String {
    func escapedString() -> String? {
        ...
        print(allowedCharacterSet.contains("/"))
